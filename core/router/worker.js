...
handlers.set('compute-run', async (payload) => {
  // Generic compute task — payload.fn is a serialized function string
  // For safety, this is a placeholder. Real implementation would
  // execute within a sandboxed context.
  const { taskType: subType, data } = payload
  return { subType, processed: true, dataSize: JSON.stringify(data).length }
})
